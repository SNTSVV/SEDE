...
class Helper(object):
        ...
        if self.assignFlag:
            ...
            for assMode in assignments:
                ...
                if not exists(join(self.improveDataPath)):
                    os.makedirs(join(self.improveDataPath))
                    ieeDV.labelHPDimages(self.improveDataNpy, self.improveDataSet, join(self.improveDataPath), "I", 1)
                if self.KP:
    ...
    def train(self):
        retrainModule.genericTrain(self.outputPath, self.datasetName, self.Epochs)
    ...
    def saveResult(self):
        ...
        if self.saveTestFlag:
            ...
            if self.Alex:
                retrainModule.alexTest(self.modelPath, self.testDataSet, None, self.datasetName, self.DNN, True,
                                       self.testCSV)
        #print("Renaming Unsafe Images")
        #RQ1.addNoise(self.caseFile)
        #RQ1.IEERQ1_U(self.caseFile)
        ...
        if self.maxClust > 500:
            self.maxClust = 500
        print("TrainingSet Size:", trainTotal, "TrainingSet accuracy:",
              str(((1 - (trainError / trainTotal)) * 100.00))[0:6] + "%")
        print("TestSet Size:", testTotal, "TestSet accuracy:",
              str(((1 - (testError / testTotal)) * 100.00))[0:6] + "%")
