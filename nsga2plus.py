...
class NSGA2Plus(GeneticAlgorithm):
...
class RankAndCrowdingSurvival(Survival):
    ...
    def _do(self, problem, pop, n_survive, D=None, **kwargs):
        ...
        #F_update = problem._evaluate(pop.get("X"), None)
...
def calc_crowding_distance(F, filter_out_duplicates=True):
    ...
    if n_points <= 2:
        ...
        # edited by fabrizio
        maxi = np.max(_F, axis=0)
        #dist = np.row_stack([_F, np.full(n_obj, maxi)]) - np.row_stack([np.full(n_obj, -np.inf), _F])
        dist = np.row_stack([_F, np.full(n_obj, 0.0)]) - np.row_stack([np.full(n_obj, -np.inf), _F])
        # calculate the norm for each objective - set to NaN if all values are equal
        norm = maxi - np.min(_F, axis=0)
        norm[norm == 0] = np.nan
...
parse_doc_string(NSGA2Plus.__init__)
