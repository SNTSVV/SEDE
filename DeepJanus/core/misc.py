...
def evaluate_sparseness(ind: Individual, pop: Set[Individual], ux: list, lx: list):
    elements = pop - {ind}
    if len(elements) == 0:
        return 1.0

    #closest_element_dist = closest_elements(elements, ind, lambda a, b: a.distance(b))[0]
    closest_element_dist = closest_elements(elements, ind, lambda a, b: doParamDist(a.params, b.params, ux, lx))[0]
